...
export const Navbar = () => {
	...
	return (
		<nav className="navbar border-body navbar-expand-sm">
					...
					<div className="btn-group mx-4 y-2">
						...
						<ul className="dropdown-menu dropdown-menu-end dropdown-menu-dark">
							{store.favorites && store.favorites.length > 0 && store.favorites.map((item, index) => (
								<li key={index} className="fav-li-item"><button className="dropdown-item" type="button"><a className="dropdown-item" href="#">({item.type}) {item.name}</a><i className="bi bi-trash3"></i></button></li>
							))}
						</ul>
					</div>
				</div>
